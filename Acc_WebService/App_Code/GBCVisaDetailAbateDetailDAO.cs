...
public class GBCVisaDetailAbateDetailDAO : GBCVisaDetailAbateDetail_Interface
{
    private const string ESTIMATE_MONEY_ABATE_STMT =
        "select ISNULL(sum(Amount),0) as 估列沖銷數 from VouDetail where FundNo=@基金代碼 and AccYear=@F_傳票年度 and DC='借' and SubNo='2125' and RelatedVouNo = @F_傳票號碼 + '-' + @明細號";

    private const string ESTIMATE_MONEY_STMT =
        "select ISNULL(sum(Amount),0) as 已估列數 from VouDetail where FundNo=@基金代碼 and AccYear=@F_傳票年度 and DC='貸' and SubNo='2125' and AccMainNo=ISNULL((select AccMainno from VouMain where FundNo=@基金代碼 and AccYear=@F_傳票年度 and VouNo=@F_傳票號碼),'0') and DetailNo=@F_明細號";
    ...
    private const string FILL_VOU_DTL_2_STMT =
        "update GBCVisaDetailAbateDetail set F_傳票明細號2=@F_傳票明細號1 where 基金代碼=@基金代碼 and PK_會計年度=@PK_會計年度 and PK_動支編號=@PK_動支編號 and PK_種類=@PK_種類 and PK_次別=@PK_次別 and PK_明細號=@PK_明細號";
    ...
    private const string PREPAY_MONEY_ABATE_STMT =
        "select ISNULL(sum(Amount),0) as 預付轉正數 from VouDetail where FundNo=@基金代碼 and AccYear=@F_傳票年度 and DC='貸' and SubNo='1154' and RelatedVouNo = @F_傳票號碼 + '-' + @明細號";

    private const string PREPAY_MONEY_STMT =
        "select ISNULL(sum(Amount),0) as 已預付數 from VouDetail where FundNo=@基金代碼 and AccYear=@F_傳票年度 and DC='借' and SubNo='1154' and AccMainNo=ISNULL((select AccMainno from VouMain where FundNo=@基金代碼 and AccYear=@F_傳票年度 and VouNo=@F_傳票號碼),'0') and DetailNo=@F_明細號";
    ...
    /// <returns></returns>
    public int EstimateMoney(string accNo, string accYear, string vouNo, string vouNoDtl)
    {
        int estimateMoney = 0;
        ...
        SqlCommand com = new SqlCommand(ESTIMATE_MONEY_STMT, con, transaction);
        com.Parameters.AddWithValue("@基金代碼", accNo);
        com.Parameters.AddWithValue("@F_傳票年度", accYear);
        com.Parameters.AddWithValue("@F_傳票號碼", vouNo);
        com.Parameters.AddWithValue("@F_明細號", vouNoDtl);

        try
        {
            estimateMoney = int.Parse(com.ExecuteScalar().ToString());
            transaction.Commit();
            con.Close();
        }
    ...
    public List<VouDetailVO> FindEstimateVouNo(Vw_GBCVisaDetail vw_GBCVisaDetail)
    {
        ...
        while (dr.Read())
        {
            ...
            vouDetailVO.傳票年度 = dr["F_傳票年度"].ToString();
    ...
    /// <param name="vw_GBCVisaDetail"></param>
    /// <returns></returns>
    public List<VouDetailVO> FindPayVouNo(Vw_GBCVisaDetail vw_GBCVisaDetail)
    {
        List<VouDetailVO> vouNoList = new List<VouDetailVO>();
        VouDetailVO vouDetailVO = null;
        SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["SqlDbConnStr"].ConnectionString);
        SqlCommand com = new SqlCommand(FIND_VOUNO_PAY_STMT, con);
        com.Parameters.AddWithValue("@基金代碼", vw_GBCVisaDetail.基金代碼);
        com.Parameters.AddWithValue("@PK_會計年度", vw_GBCVisaDetail.PK_會計年度);
        com.Parameters.AddWithValue("@PK_動支編號", vw_GBCVisaDetail.PK_動支編號);
        com.Parameters.AddWithValue("@PK_種類", "核銷");
    ...
    /// <returns></returns>
    public List<GBCVisaDetailAbateDetailVO> FindGBCReco(string fundNo, string acmWordNum)
    {
    ...
    /// <param name="種類"></param>
    /// <param name="次別"></param>
    /// <param name="明細號"></param>
    /// <param name="傳票明細號"></param>
    public void FillVouDtl1(string 基金代碼, string 會計年度, string 動支編號, string 種類, string 次別, string 明細號, int 傳票明細號)
    {

        SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["SqlDbConnStr"].ConnectionString);
        SqlCommand com = new SqlCommand(FILL_VOU_DTL_1_STMT, con);
        com.Parameters.AddWithValue("@基金代碼", 基金代碼);
        com.Parameters.AddWithValue("@PK_會計年度", 會計年度);
        com.Parameters.AddWithValue("@PK_動支編號", 動支編號);
        com.Parameters.AddWithValue("@PK_種類", 種類);
        com.Parameters.AddWithValue("@PK_次別", 次別);
        com.Parameters.AddWithValue("@PK_明細號", 明細號);
        com.Parameters.AddWithValue("@F_傳票明細號1", 傳票明細號);
        con.Open();
    ...
    /// <param name="明細號"></param>
    /// <param name="傳票明細號"></param>
    public void FillVouDtl2(string 基金代碼, string 會計年度, string 動支編號, string 種類, string 次別, string 明細號, int 傳票明細號)
    {

        SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["SqlDbConnStr"].ConnectionString);
        SqlCommand com = new SqlCommand(FILL_VOU_DTL_2_STMT, con);
        com.Parameters.AddWithValue("@基金代碼", 基金代碼);
        com.Parameters.AddWithValue("@PK_會計年度", 會計年度);
        com.Parameters.AddWithValue("@PK_動支編號", 動支編號);
        com.Parameters.AddWithValue("@PK_種類", 種類);
    ...
    public void Insert(Vw_GBCVisaDetail vw_GBCVisaDetail)
    {
        ...
        try
        {            
            com = new SqlCommand(INSERT_STMT, conn, transtation);
            com.Parameters.AddWithValue("@基金代碼", vw_GBCVisaDetail.基金代碼);
            com.Parameters.AddWithValue("@PK_會計年度", vw_GBCVisaDetail.PK_會計年度);
            com.Parameters.AddWithValue("@PK_動支編號", vw_GBCVisaDetail.PK_動支編號);
            com.Parameters.AddWithValue("@PK_種類", vw_GBCVisaDetail.PK_種類);
    ...
    /// <param name="vouNoDtl"></param>
    /// <returns></returns>
    public int PrePayMoney(string accNo, string accYear, string vouNo, string vouNoDtl)
    {
        SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["SqlDbConnStr"].ConnectionString);
    ...
    public void UpdateVouNo2(GBCVisaDetailAbateDetailVO gbcVisaDetailAbateDetailVO)
    {
        ...
        //com.Parameters.AddWithValue("@傳票明細號1", gbcVisaDetailAbateDetailVO.getF_傳票明細號1());
        //com.Parameters.AddWithValue("@製票日期1", gbcVisaDetailAbateDetailVO.getF_製票日期1());
        com.Parameters.AddWithValue("@傳票號2", gbcVisaDetailAbateDetailVO.getF_傳票號1());
        com.Parameters.AddWithValue("@傳票明細號2", gbcVisaDetailAbateDetailVO.getF_傳票明細號1());
        com.Parameters.AddWithValue("@製票日期2", gbcVisaDetailAbateDetailVO.getF_製票日期1());
